...
5. **Review and merge.** The review ruleset requires one CODEOWNERS review, and the quality ruleset requires the shared `pr-checks` status checks plus the local `Validate Listings` job (which re-runs the zod schema against every listing). Merge to `main` triggers the Cloudflare Pages build and deploy.
